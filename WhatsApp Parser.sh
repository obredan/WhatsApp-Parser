...
if [ "$(id -u)" = "0" ]; then
	sleep 2
	clear
	echo -e -n "$blueColour-> Would you like to install FatRat for BackDoor creation?$endColour$redColour <Y/N>$endColour: "
	read answer
	echo " "
	...
	echo -e -n "$blueColour->  Would you like to install the Yowsup library?$endColour$redColour <Y/N>$endColour: "
	read answer
	echo " "
	...
	echo -e "\n$redColour-> You must be logged in as root$endColour\n"
fi
